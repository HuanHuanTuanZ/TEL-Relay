...
void hud()
{
	...
	if(tele_flag)
	{
		if(crsf_yaw>0)yaw=crsf_yaw*187/32767;
		else
		{
			yaw=(32767+crsf_yaw)*172/30045+187;
			if(yaw>359)yaw=359;
		}
		roll=-crsf_roll*180/32768;
		pitch=-crsf_pitch*180/32768;
		alt=crsf_altitude;
		speed=crsf_ground_speed;
	}
	else
	{
		yaw+=(CH_out[3]-1500)/50;
		if(yaw<0)yaw+=360;
		else if(yaw>360)yaw-=360;
		alt+=(CH_out[2]-1500)/50;	
		roll=-(CH_out[0]-1500)*180/500;		
		pitch=-(CH_out[1]-1500)*90/500;
		speed=(CH_out[0]+CH_out[1]-2000)/10;
	}
	
	XUI_ground(roll,pitch,LIGHTBLUE8,BROWN8);
	XUI_yaw(yaw,WHITE8,DARKBLUE8);	
	XUI_alt(alt,WHITE8,DARKBLUE8);
	XUI_speed(speed,WHITE8,DARKBLUE8);	
...
void hud2()
{
	...
	XUI_ground(roll,pitch,GREEN8,GREEN8);
	XUI_yaw(yaw,GREEN8,BLACK8);	
	XUI_alt(alt,GREEN8,BLACK8);
	XUI_speed(speed,GREEN8,BLACK8);	
	...
	if(hud_type2==1)
	{
		XUI_roll(roll,GREEN8);
		XUI_pitch(pitch,roll,GREEN8,GREEN8);
		XUI_line(76,42,80,38,GREEN8);
		XUI_line(80,38,84,42,GREEN8);		
	}	
	else XUI_3d(3.1416f-pitch/100.0f,0,3.1416f-roll/100.0f,40,GREEN8);	
