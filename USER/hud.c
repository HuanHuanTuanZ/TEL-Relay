...
void XUI_yaw(u16 yaw, u8 c1, u8 c2)
{
	...
	for (i = 0; i < 11; i++)
	{
		...
		if (x >= -20 && x <= 180)
		{
			...
			if (temp == 0)
				XUI_char8(x - 3, 0, 'N', 1, c2, 0);
			else if (temp == 90)
				XUI_char8(x - 3, 0, 'E', 1, c2, 0);
			else if (temp == 180)
				XUI_char8(x - 3, 0, 'S', 1, c2, 0);
			else if (temp == 270)
				XUI_char8(x - 3, 0, 'W', 1, c2, 0);
			else
				XUI_num8(x - len(temp) * 3, 0, temp, len(temp), 1, c1, 0);
		}
	}
...
void XUI_speed(int16 speed, u8 c1, u8 c2)
{
	...
	for (i = 0; i < 11; i++)
	{
		...
		if (y >= 20 && y <= 60)
		{
			...
			{
				temp = -temp;
				XUI_char8(24, y - 4, '-', 1, c1, 0);
			} // RAM_ShowChar(97,320-y-6,'-',12);}
			else
				XUI_num8(30 - len(temp) * 6, y - 4, temp, len(temp), 1, c1, 0);
		}
	}
...
void XUI_alt(int16 alt, u8 c1, u8 c2)
{
	...
	for (i = 0; i < 11; i++)
	{
		...
		if (y >= 20 && y <= 60)
		{
			...
			{
				temp = -temp;
				XUI_char8(130, y - 4, '-', 1, c1, 0);
				XUI_num8(135, y - 4, temp, len(temp), 1, c1, 0);
			} // RAM_ShowChar(97,320-y-6,'-',12);}
			else
				XUI_num8(130, y - 4, temp, len(temp), 1, c1, 0);
		}
	}
...
void XUI_line_roll0(int16 x1, int16 y1, int16 x2, int16 y2, int16 roll)
{
	...
	static int16 y0 = 40;
	roll += 180;
	x = x0 + cos(acos((x1 - x0) / (sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0)))) + roll * pi / 180) * sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0));
	if (y1 > y0)
	{
...
void hud()
{
	...
	if (hud_type == 1)
	{
		...
		XUI_3d(3.1416f - hud_pitch * pi / 180.0, 0, 3.1416f - hud_roll * pi / 180.0f, 40, WHITE8);
...
void hud2()
{
	...
	if (hud_type2 == 1)
	{
		...
		XUI_3d(3.1416f - hud_pitch / 100.0f, 0, 3.1416f - hud_roll / 100.0f, 40, GREEN8);
